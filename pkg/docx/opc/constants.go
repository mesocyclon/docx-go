...
// Used when building [Content_Types].xml.
...
// IsDefaultContentType returns true if (ext, contentType) is in the default map.
// Extension comparison is case-insensitive.
func IsDefaultContentType(ext, contentType string) bool {
	ct, ok := DefaultContentTypes[strings.ToLower(ext)]
	return ok && ct == contentType
}
